...
page_menu(
  input_buttongroup(
    "Select which", c("district", "county"),
    display = c("District", "County"), id = "scale"
  ),
  input_select(
    "Selection",
    options = "levels", variable = "ID",
    depends = "scale", id = "scope", reset_button = TRUE
  ),
  input_buttongroup("Select Map Shapes", "dataset", default = "district", id = "shapes"),
  input_checkbox("Select a Region Type", options = c("rural", "mixed", "urban"), id = "region_type"),
  input_slider("Select a Year", variable = "year", id = "year"),
  input_select(
    "Select a Variable",
    options = "variables", id = "variable",
